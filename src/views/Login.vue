...
async function login() {
	let data = await doLoign({
		username: account.value,
		password: password.value,
	});
	if (data.status) return alert("账号或密码错误");
	localStorage.setItem("userInfo", JSON.stringify(data.meta.data));
	localStorage.setItem("token", data.meta.data.token);
	alert("登陆成功");
	router.push("/");
}
</script>
